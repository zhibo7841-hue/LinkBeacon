...
Status: design baseline / planning, 2026-09-26. No runtime implementation or permission change is made by this audit. Android 12–16 behavior below is an API contract and test plan, **not** a claim that v0.9 has passed device acceptance.
...
1. **Task A — Platform + Domain Core:** final manifest/permission decision for actual APIs, typed platform adapter/models, timestamp/security/channel/identity helpers, fake fixtures and unit tests.
2. **Task B — UI + Permissions:** Tools entry, one bilingual page, Current/Nearby/Channels overview, user-initiated refresh, denial/settings and freshness UX, lifecycle and instrumentation tests.
3. **Task C — Channel Visualization (optional):** only if evidence/readability review justifies it; Compose Canvas with a text-equivalent view, no best-channel claim.
4. **Task D — Real-device + Performance Regression:** Sony Android 12 and Android 16 development checks, permission/location-off restoration, cache/throttle/rotation/network-change cases, hundreds-of-AP fixture performance. Freeze/RC planning is a later authorization.
